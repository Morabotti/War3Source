...
//skill 1
new Float:ShockWaveChance[9]={0.0,0.20,0.22,0.24,0.26,0.28,0.30,0.32,0.35};
...
new String:StompSnd[]="*mora-wcs/war3source/morabotti/growl3.mp3";
new String:StompSnd2[]="*mora-wcs/weapons/mortar/mortar_explode1.mp3";
new String:StompSnd_FullPath[]="sound/mora-wcs/war3source/morabotti/growl3.mp3";
new String:StompSnd2_FullPath[]="sound/mora-wcs/weapons/mortar/mortar_explode1.mp3";
new String:PlasmaSprite_FullPath[] = "materials/mora-wcs/sprites/plasma.vmt";
new String:PlasmaSpriteVTF_FullPath[] = "materials/mora-wcs/sprites/plasma.vtf";
...
public OnWar3LoadRaceOrItemOrdered(num)
{
	if(num==300)
	{
		thisRaceID=War3_CreateNewRace("Tauren Chieftain","taurenc");
...
public OnMapStart()
{
	AddFileToDownloadsTable(PlasmaSprite_FullPath);
	AddFileToDownloadsTable(PlasmaSpriteVTF_FullPath);
	FlameSprite=PrecacheModel("materials/mora-wcs/sprites/plasma.vmt");
	...
	AddFileToDownloadsTable(StompSnd_FullPath);
	AddFileToDownloadsTable(StompSnd2_FullPath);
	
	PrecacheSoundAny(StompSnd);
	PrecacheSoundAny(StompSnd2);
}
	...
	if(War3_GetRace(client)==thisRaceID && ability==0 && pressed && IsPlayerAlive(client))
	{
		...
		if(skill_level>0)
		{
			if(War3_SkillNotInCooldown(client,thisRaceID,SKILL_WARSTOMP,true))
			{
				if(!Silenced(client))
				{
					EmitSoundToAllAny(StompSnd,client);
...
{
	EmitSoundToAllAny(StompSnd2,client);
}
		...
		if(vteam!=ateam)
		{
			...
			if(race_attacker==thisRaceID && skill_level_shockwave>0 && !Silenced(attacker))
			{
				if(GetRandomFloat(0.0,1.0)<=ShockWaveChance[skill_level_shockwave] && !W3HasImmunity(victim,Immunity_Skills))
				{
					...
					epos[2]+=35;
					TE_SetupBeamPoints(spos, epos, FlameSprite, FlameSprite, 1, 5, 1.5, 20.0, 20.0, 2, 4.0, {255,80,20,255}, 30);
					TE_SendToAll();
					War3_ShakeScreen(victim,2.0,50.0,40.0);
					PrintHintText(victim, "Tauren Chieftain shockwaved you.");
					PrintHintText(attacker, "Your have shockwaved target");
...
public OnWar3EventSpawn(client)
{
	...
	if(race==thisRaceID)
	{
		...
		TE_SetupBeamRingPoint(SpawnPos, 1.0, 500.0, BeamSprite, HaloSprite, 0, 5, 1.0, 50.0, 1.0, {84,84,84,250}, 60, 0);
		TE_SendToAll();
	}
}
