...
public Action:StuckCmd(client, Args)
{
	if(IsPlayerAlive(client))
	{
		if(AreYouStuck(client))
		{
			...
			PrintToChat(client, "[-WCS-] You're not actually stuck");
	}
	else
		PrintToChat(client, "[-WCS-] You're not alive");
}
...
{	
	...
	if(GetVectorDistance(vecOrigin, vecOriginAfter) < 10.0)
	{
		if(testID == 0)
			...
			PrintToChat(client,"[-WCS-] Sorry, we can't fix your position");
		else
		{
			PrintToChat(client, "[-WCS-] You're not stuck anymore");
		}
	}
	else
		PrintToChat(client, "[-WCS-] You're not actually stuck");
