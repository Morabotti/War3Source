...
// Chance/Data Arrays
new ElectricTideMaxDamage[9]={0,20,40,60,80,100,120,140,160};
new Float:ElectricTideRadius=500.0;
new Float:AbilityCooldownTime=15.0;

new ConduitPerHit[9]={0,1,1,2,2,3,3,4,4};
new ConduitDuration=10;
new ConduitCooldown=15;
new ConduitMaxHeal[9]={0,2,4,6,8,10,12,14,15};

new Float:StaticHealPercent[9]={0.0,0.10,0.20,0.30,0.40,0.50,0.60,0.70,0.80};
new StaticHealRadius=800;

new OverloadDuration=60; //HIT TIMES, DURATION DEPENDS ON TIMER
new OverloadRadius=550;
new OverloadDamagePerHit[9]={0,2,3,4,5,6,7,8,10};
new Float:OverloadDamageIncrease[9]={1.0,1.005,1.01,1.015,1.020,1.025,1.030,1.035,1.040};
////
...
new String:taunt1[]="*mora-wcs/war3source/cd/feeltheburn2.mp3";
new String:taunt2[]="*mora-wcs/war3source/cd/feeltheburn3.mp3";
new String:overload1[]="*mora-wcs/war3source/cd/overload2.mp3";
new String:overloadzap[]="*mora-wcs/war3source/cd/overloadzap.mp3";
new String:overloadstate[]="*mora-wcs/war3source/cd/ultstate.mp3";

new String:taunt1_FullPath[]="sound/mora-wcs/war3source/cd/feeltheburn2.mp3";
new String:taunt2_FullPath[]="sound/mora-wcs/war3source/cd/feeltheburn3.mp3";
new String:overload1_FullPath[]="sound/mora-wcs/war3source/cd/overload2.mp3";
new String:overloadzap_FullPath[]="sound/mora-wcs/war3source/cd/overloadzap.mp3";
new String:overloadstate_FullPath[]="sound/mora-wcs/war3source/cd/ultstate.mp3";

// Effects
new BeamSprite,HaloSprite,ElectSprite/*,LightningSprite*/;
...
public OnWar3LoadRaceOrItemOrdered(num)
{
    if(num==90)
    {
        ...
        SKILL_TIDE=War3_AddRaceSkillT(thisRaceID,"ElectricTide",false,8);
        SKILL_CONDUIT=War3_AddRaceSkillT(thisRaceID,"CorruptedConduit",false,8);
        SKILL_STATIC=War3_AddRaceSkillT(thisRaceID,"StaticDischarge",false,8);
        ULT_OVERLOAD=War3_AddRaceSkillT(thisRaceID,"Overload",true,8);
...
public OnMapStart()
{
	AddFileToDownloadsTable(taunt1_FullPath);
	AddFileToDownloadsTable(taunt2_FullPath);
	AddFileToDownloadsTable(overload1_FullPath);
	AddFileToDownloadsTable(overloadzap_FullPath);
	AddFileToDownloadsTable(overloadstate_FullPath);
	AddFileToDownloadsTable("materials/mora-wcs/sprites/xbeam2.vmt");
	AddFileToDownloadsTable("materials/mora-wcs/sprites/xbeam2.vtf");
	
	//LightningSprite = PrecacheModel( "materials/mora-wcs/sprites/lgtning.vmt" );
	ElectSprite=PrecacheModel("materials/mora-wcs/sprites/xbeam2.vmt");
	BeamSprite=War3_PrecacheBeamSprite();
	HaloSprite=War3_PrecacheHaloSprite();
	
	PrecacheSoundAny(taunt1);
	PrecacheSoundAny(taunt2);
	PrecacheSoundAny(overload1);
	PrecacheSoundAny(overloadzap);
	PrecacheSoundAny(overloadstate);
}
    ...
    if(/*War3_GetRace(client)==thisRaceID &&*/ ability==0 && pressed && ValidPlayer(client, true))
    {
        ...
        if(skill_level>0)
        {
            if(!Silenced(client)&&War3_SkillNotInCooldown(client,thisRaceID,SKILL_TIDE,true))
            {
                ...
                ElectricTideOrigin[client][2]+=40.0;
                ...
                TE_SetupBeamRingPoint(ElectricTideOrigin[client], 20.0, ElectricTideRadius+50, BeamSprite, HaloSprite, 0, 5, 0.5, 10.0, 1.0, {0,102,204,133}, 60, FBEAM_ISACTIVE);
                TE_SendToAll();
                ...
                EmitSoundToAllAny(taunt1, client);
                EmitSoundToAllAny(taunt1, client);
                EmitSoundToAllAny(taunt2, client);
...
public Action:SecondRing(Handle:timer,any:userid)
{
    ...
    TE_SetupBeamRingPoint(ElectricTideOrigin[client], ElectricTideRadius+50,20.0, BeamSprite, HaloSprite, 0, 5, 0.5, 10.0, 1.0, {0,102,204,133}, 60, FBEAM_ISACTIVE);
    TE_SendToAll();
}
    ...
    if(War3_GetRace(client) == thisRaceID)
     {
        ...
        TE_SetupBeamPoints(spawn_pos, spawn_pos2, ElectSprite, ElectSprite, 0, 60, 4.0, 20.0, 1.0, 0, 5.0, {0,76,153,255}, 40);
        TE_SendToAll(0.0);
        TE_SetupBeamPoints(spawn_pos, spawn_pos2, ElectSprite, ElectSprite, 2, 50, 4.0, 15.0, 1.0, 0, 5.0, {153,204,255,255}, 40);
        TE_SendToAll(0.0);
     }
}
    ...
    if(race==thisRaceID && pressed && IsPlayerAlive(client))
    {
        ...
        if(skill>0)
        {
            if(!Silenced(client)&&War3_SkillNotInCooldown(client,thisRaceID,ULT_OVERLOAD,true))
            {
				...
				EmitSoundToAllAny(overload1, client);
				EmitSoundToAllAny(overload1, client);
				EmitSoundToAllAny(overloadstate, client);
...
public Action:UltimateLoop(Handle:timer,any:userid)
{
    ...
    if(ValidPlayer(attacker) && UltimateZapsRemaining[attacker]>0&&IsPlayerAlive(attacker))
    {
        ...
        if(besttarget>0){
			...
			TE_SetupBeamPoints(pos,otherpos,ElectSprite,HaloSprite,0,35,0.15,6.0,5.0,0,1.0,{0,102,204,255},2);
			TE_SendToAll();
			...
			EmitSoundToAllAny(overloadzap, attacker);
			EmitSoundToAllAny(overloadzap, attacker);
			EmitSoundToAllAny(overloadzap, besttarget);
			EmitSoundToAllAny(overloadzap, besttarget);
...
public Action:UltStateSound(Handle:t,any:attacker){
    ...
    if(ValidPlayer(attacker,true)&&UltimateZapsRemaining[attacker]>0){
    	EmitSoundToAllAny(overloadstate, attacker);
