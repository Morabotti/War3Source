/*
* War3Source Race - Jigsaw
* 
* File: War3Source_Jigsaw.sp
...
// Cube
new CubeDMG[9] =  { 0, 1, 2, 2, 3, 3, 4, 4, 5 };
new Float:CubeRange[9] = {0.0, 540.0, 540.0, 540.0, 540.0, 540.0, 540.0, 540.0, 540.0};

// Razor Wire
new Float:RazorDMG[9]={0.0, 0.07, 0.08, 0.09, 0.1, 0.11, 0.12, 0.13, 0.14};
new Float:RazorRange[9] = {0.0, 540.0, 540.0, 540.0, 540.0, 540.0, 540.0, 540.0, 540.0};
new Float:lastLocation[MAXPLAYERS][3];

// Furnace Trap
new Float:FurnaceRange[9] = {0.0, 540.0, 540.0, 540.0, 540.0, 540.0, 540.0, 540.0, 540.0};

// Carousel
new CarouselDMG[9] = { 0, 15, 20, 25, 30, 35, 40, 45, 50 };

// Blade Table
new BladeTableDMG[9] =  { 0, 1, 2, 2, 3, 3, 4, 4, 5 };
new BladeTableSacrificeDMG[9] =  { 0, 15, 20, 25, 30, 35, 40, 45, 50 };
new Float:BladeTableRange[9] = {0.0, 540.0, 540.0, 540.0, 540.0, 540.0, 540.0, 540.0, 540.0};

// Reverse Beartrap
new Float:BeartrapTime[9] =  { 0.0, 40.0, 35.0, 30.0, 25.0, 23.0, 20.0, 17.0, 15.0 };
...
// Soundy
new String:Resp1Snd[]="*mora-wcs/war3source/jigsaw/resp1.mp3";
new String:Resp1Snd_FullPath[]="sound/mora-wcs/war3source/jigsaw/resp1.mp3";
new String:Resp2Snd[]="*mora-wcs/war3source/jigsaw/resp2.mp3";
new String:Resp2Snd_FullPath[]="sound/mora-wcs/war3source/jigsaw/resp2.mp3";
new String:LaughSnd[]="*mora-wcs/war3source/jigsaw/laugh.mp3";
new String:LaughSnd_FullPath[]="sound/mora-wcs/war3source/jigsaw/laugh.mp3";
new String:LiveOrDieSnd[]="*mora-wcs/war3source/jigsaw/liveordie.mp3";
new String:LiveOrDieSnd_FullPath[]="sound/mora-wcs/war3source/jigsaw/liveordie.mp3";
new String:WinSnd[]="*mora-wcs/war3source/jigsaw/win.mp3";
new String:WinSnd_FullPath[]="sound/mora-wcs/war3source/jigsaw/win.mp3";
new String:GameOverSnd[]="*mora-wcs/war3source/jigsaw/gameover.mp3";
new String:GameOverSnd_FullPath[]="sound/mora-wcs/war3source/jigsaw/gameover.mp3";
new String:SacrificeSnd[]="*mora-wcs/war3source/jigsaw/sacrifice.mp3";
new String:SacrificeSnd_FullPath[]="sound/mora-wcs/war3source/jigsaw/sacrifice.mp3";
new String:BeartrapSnd[]="*mora-wcs/war3source/jigsaw/beartrap.mp3";
new String:BeartrapSnd_FullPath[]="sound/mora-wcs/war3source/jigsaw/beartrap.mp3";
new String:BloodSnd[]="*mora-wcs/war3source/jigsaw/blood.mp3";
new String:BloodSnd_FullPath[]="sound/mora-wcs/war3source/jigsaw/blood.mp3";
new String:CarouselSnd[]="*mora-wcs/war3source/jigsaw/carousel.mp3";
new String:CarouselSnd_FullPath[]="sound/mora-wcs/war3source/jigsaw/carousel.mp3";
new String:RazorSnd[]="*mora-wcs/war3source/jigsaw/razorwire.mp3";
new String:RazorSnd_FullPath[]="sound/mora-wcs/war3source/jigsaw/razorwire.mp3";
...
public OnWar3LoadRaceOrItemOrdered(num)
{
	...
	SKILL_CUBE=War3_AddRaceSkill(thisRaceID,"Cube","Traps your enemy into a Cube and drowns him. Target needs 1 frag to escape.",false,8);
	SKILL_RAZOR=War3_AddRaceSkill(thisRaceID,"Razor Wire","Entwines your enemy with razor wire. Target needs 1 frag to escape.",false,8);
	SKILL_FURNACE=War3_AddRaceSkill(thisRaceID,"Furnace","Burns your enemy. Target needs 2 frags to escape.",false,8);
	SKILL_CAROUSEL=War3_AddRaceSkill(thisRaceID,"Carousel","Shots your enemy with a shotgun.",false,8);
	SKILL_BLADETABLE=War3_AddRaceSkill(thisRaceID,"Blade Table","Damages your enemy. Target needs self-sacrifice or 2 frags to escape.",false,8);
	ULT_BEARTRAP=War3_AddRaceSkill(thisRaceID,"Reverse Beartrap (death)","Traps your enemy in Reverse Beartrap. Target needs 2 frags or 1 TK to escape.",true,8); 
...
{
	// Soundy
	AddFileToDownloadsTable(Resp1Snd_FullPath);
	AddFileToDownloadsTable(Resp2Snd_FullPath);
	AddFileToDownloadsTable(LaughSnd_FullPath);
	AddFileToDownloadsTable(LiveOrDieSnd_FullPath);
	AddFileToDownloadsTable(WinSnd_FullPath);
	AddFileToDownloadsTable(GameOverSnd_FullPath);
	AddFileToDownloadsTable(SacrificeSnd_FullPath);
	AddFileToDownloadsTable(BeartrapSnd_FullPath);
	AddFileToDownloadsTable(BloodSnd_FullPath);
	AddFileToDownloadsTable(CarouselSnd_FullPath);
	AddFileToDownloadsTable(RazorSnd_FullPath);
	
	BeamSprite = PrecacheModel( "materials/mora-wcs/sprites/lgtning.vmt" );
	
	PrecacheSoundAny(Resp1Snd);
	PrecacheSoundAny(Resp2Snd);
	PrecacheSoundAny(LaughSnd);
	PrecacheSoundAny(LiveOrDieSnd);
	PrecacheSoundAny(WinSnd);
	PrecacheSoundAny(GameOverSnd);
	PrecacheSoundAny(SacrificeSnd);
	PrecacheSoundAny(BeartrapSnd);
	PrecacheSoundAny(BloodSnd);
	PrecacheSoundAny(CarouselSnd);
	PrecacheSoundAny(RazorSnd);
...
{    
	...
	if(race==thisRaceID)
	{
		...
		if(resp == 1)
		{
			EmitSoundToAllAny(Resp1Snd,client);
		}
		if(resp == 2)
		{
			EmitSoundToAllAny(Resp2Snd,client);
		}
	...
	if( victim > 0 && attacker > 0 && attacker != victim)
	{
		if(vteam != ateam)
		{
			...
			if(War3_GetRace(victim) == thisRaceID && War3_GetSkillLevel(victim,thisRaceID,ULT_BEARTRAP) > 0 && !W3HasImmunity(attacker, Immunity_Ultimates))
			{
				...
				EmitSoundToAllAny(BeartrapSnd,attacker);
			}
		}
		else
		{
			if(IsTrapped[attacker] == 6) // Reverse Beartrap
			{
				...
				FragCounter[attacker] = 0; // reset
				TrapDelayer[attacker] = 0; // reset
				Instructions[attacker] = 0; // reset
				EmitSoundToAllAny(WinSnd,attacker); // sound
				PrintHintText(attacker, "You've escaped from the Trap");
			}			
		}
		...
		if(IsTrapped[victim] == 1 || IsTrapped[victim] == 2 || IsTrapped[victim] == 3 || IsTrapped[victim] == 5 || IsTrapped[victim] == 6) // if trapped and die
		{
			...
			if(death == 1)
			{
				EmitSoundToAllAny(LaughSnd,victim);
			}
			if(death == 2)
			{
				EmitSoundToAllAny(GameOverSnd,victim);
			}
		}
	}
}
			...
			switch(IsTrapped[i])
			{
				...
				case 1: // Cube
				{
					if(FragCounter[i] != 1)
					{
						...
						if(TrapDelayer[i] == 10)
						{
							...
							else
							{
								...
								if(death == 1)
								{
									EmitSoundToAllAny(LaughSnd,i);
								}
								if(death == 2)
								{
									EmitSoundToAllAny(GameOverSnd,i);
								}
							}
							TrapDelayer[i] = 0;
						}						
					...
					else // if amount of kills reached
					{
						...
						Instructions[i] = 0; // reset
						EmitSoundToAllAny(WinSnd,i); // sound
	...
	if(!Silenced(client))
	{
		if(bIsUsed[0][client] == false)
		{
			...
			if(ValidPlayer(cubeTarget,true) && !W3HasImmunity(cubeTarget,Immunity_Skills) && IsTrapped[cubeTarget] == 0)
			{
				...
				EmitSoundToAllAny(LiveOrDieSnd,cubeTarget);
				
				new Float:aPos[3];
				new Float:bPos[3];
				GetClientAbsOrigin(client, aPos);
				GetClientAbsOrigin(cubeTarget, bPos);
				aPos[2] += 20;
				bPos[2] += 20;
				TE_SetupBeamPoints(aPos, bPos, BeamSprite, 0, 0, 0, 1.0, 4.0, 4.0, 0, 0, { 51, 153, 255, 255 }, 0);
				TE_SendToAll();
	...
	if(!Silenced(client))
	{
		if(bIsUsed[1][client] == false)
		{
			...
			if(ValidPlayer(razorTarget,true) && !W3HasImmunity(razorTarget,Immunity_Skills) && IsTrapped[razorTarget] == 0)
			{
				...
				EmitSoundToAllAny(RazorSnd,razorTarget);
				
				new Float:aPos[3];
				new Float:bPos[3];
				GetClientAbsOrigin(client, aPos);
				GetClientAbsOrigin(razorTarget, bPos);
				aPos[2] += 20;
				bPos[2] += 20;
				TE_SetupBeamPoints(aPos, bPos, BeamSprite, 0, 0, 0, 1.0, 4.0, 4.0, 0, 0, { 102, 51, 0, 255 }, 0);
				TE_SendToAll();
	...
	if(!Silenced(client))
	{
		if(bIsUsed[2][client] == false)
		{
			...
			if(ValidPlayer(furnaceTarget,true) && !W3HasImmunity(furnaceTarget,Immunity_Skills) && IsTrapped[furnaceTarget] == 0)
			{
				...
				EmitSoundToAllAny(LiveOrDieSnd,furnaceTarget);
				
				new Float:aPos[3];
				new Float:bPos[3];
				GetClientAbsOrigin(client, aPos);
				GetClientAbsOrigin(furnaceTarget, bPos);
				aPos[2] += 20;
				bPos[2] += 20;
				TE_SetupBeamPoints(aPos, bPos, BeamSprite, 0, 0, 0, 1.0, 4.0, 4.0, 0, 0, { 255, 36, 0, 255 }, 0);
				TE_SendToAll();
		...
		if(bIsUsed[3][client] == false)
		{
			...
			if(carouselTarget > 0 && ValidPlayer(carouselTarget,true) && !W3HasImmunity(carouselTarget,Immunity_Skills))
			{
				...
				if(GetClientHealth(carouselTarget) < damage)
				{
					...
					if(death == 1)
					{
						EmitSoundToAllAny(LaughSnd,carouselTarget);
					}
					if(death == 2)
					{
						EmitSoundToAllAny(GameOverSnd,carouselTarget);
					}				
				}		
				
				War3_DealDamage(carouselTarget,damage,TrappedBy[carouselTarget],_,"carousel",_,W3DMGTYPE_TRUEDMG);
				EmitSoundToAllAny(CarouselSnd,carouselTarget);	
				
				new Float:aPos[3];
				new Float:bPos[3];
				GetClientAbsOrigin(client, aPos);
				GetClientAbsOrigin(carouselTarget, bPos);
				aPos[2] += 20;
				bPos[2] += 20;
				TE_SetupBeamPoints(aPos, bPos, BeamSprite, 0, 0, 0, 1.0, 4.0, 4.0, 0, 0, { 255, 0, 255, 255 }, 0);
				TE_SendToAll();
	...
	if(!Silenced(client))
	{
		if(bIsUsed[4][client] == false)
		{
			...
			if(ValidPlayer(bladetableTarget,true) && !W3HasImmunity(bladetableTarget,Immunity_Skills) && IsTrapped[bladetableTarget] == 0)
			{
				...
				new Float:bPos[3];
				GetClientAbsOrigin(client, aPos);
				GetClientAbsOrigin(bladetableTarget, bPos);
				aPos[2] += 20;
				bPos[2] += 20;
				TE_SetupBeamPoints(aPos, bPos, BeamSprite, 0, 0, 0, 1.0, 4.0, 4.0, 0, 0, { 204, 255, 51, 255 }, 0);
				TE_SendToAll();
				...
				EmitSoundToAllAny(SacrificeSnd,bladetableTarget);
...
public SelectDeath(Handle:menu2, MenuAction:action, client, param2)
{
    if (action == MenuAction_Select)
    {
        ...
        {			
			...
			EmitSoundToAllAny(BloodSnd,client);
        }
        else if(StrEqual(info,"sacrifice"))
        {
			...
			if(GetClientHealth(client) < damage)
			{
				...
				if(death == 1)
				{
					EmitSoundToAllAny(LaughSnd,client);
				}
				if(death == 2)
				{
					EmitSoundToAllAny(GameOverSnd,client);
				}				
			}
