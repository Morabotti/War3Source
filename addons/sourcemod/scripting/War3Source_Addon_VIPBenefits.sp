...
public OnWar3Event(W3EVENT:event,client)
{
    if(event==OnPreGiveXPGold)
    {
        ...
        {
            new AdminFlag:flag;
            if (!FindFlagByChar('o', flag) || !FindFlagByChar('p', flag))
            {
            ...
            else
            {
				...
				{
					W3SetVar(EventArg2,RoundToCeil(FloatMul(float(W3GetVar(EventArg2)),1.20)));
					W3SetVar(EventArg3,RoundToCeil(FloatMul(float(W3GetVar(EventArg3)),1.5)));
				}
            }
        }
    }
