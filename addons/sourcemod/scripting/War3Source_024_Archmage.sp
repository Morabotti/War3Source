...
//Blizzard
new Float:BlizTime[] =  { 0.0, 6.0, 7.0, 8.0, 9.0, 10.0, 11.0, 12.0, 13.0 };
...
new Float:SummonCD[] =  { 0.0, 39.0, 37.0, 36.0, 34.0, 33.0, 32.0, 31.0, 30.0 };

//Brilliance Aura
new HealAmount[] =  { 0, 1, 1, 1, 2, 2, 3, 3, 3, 4 };
new Float:HealRD[] =  { 0.0, 125.0, 150.0, 175.0, 200.0, 225.0, 250.0, 275.0, 300.0 };

//Mass Teleport
new Float:TeleRD[] =  { 0.0, 825.0, 850.0, 875.0, 900.0, 925.0, 950.0, 975.0, 1000.0 };
...
new String:teleport_sound[] = "*mora-wcs/war3source/blinkarrival.mp3";
new String:summon_sound[] = "*mora-wcs/war3source/reincarnation.mp3";
new String:blizzard_sound[] = "*mora-wcs/war3source/moonqueen/beam.mp3";
new String:blizzardloop_sound[] = "*mora-wcs/war3source/moonqueen/beam.mp3";
new String:teleport_sound_FullPath[] = "sound/mora-wcs/war3source/blinkarrival.mp3";
new String:summon_sound_FullPath[] = "sound/mora-wcs/war3source/reincarnation.mp3";
new String:blizzard_sound_FullPath[] = "sound/mora-wcs/war3source/moonqueen/beam.mp3";
new String:blizzardloop_sound_FullPath[] = "sound/mora-wcs/war3source/moonqueen/beam.mp3";
...
public OnMapStart()
{
	AddFileToDownloadsTable(teleport_sound_FullPath);
	AddFileToDownloadsTable(summon_sound_FullPath);
	AddFileToDownloadsTable(blizzard_sound_FullPath);
	AddFileToDownloadsTable(blizzardloop_sound_FullPath);
	...
	PrecacheSoundAny(summon_sound);
	PrecacheSoundAny(teleport_sound);
	PrecacheSoundAny(blizzard_sound);
	PrecacheSoundAny(blizzardloop_sound);
...
public OnWar3LoadRaceOrItemOrdered(num)
{
	if (num == 240) {
		...
		SKILL_BLIZZARD = War3_AddRaceSkill(thisRaceID, "Blizzard (Ability)", "Summon a snowstorm", false, 8);
		SKILL_SUMMON = War3_AddRaceSkill(thisRaceID, "Summon Elemental (Ability1)", "Raise the dead allies", false, 8);
		SKILL_AURA = War3_AddRaceSkill(thisRaceID, "Diamond Aura (Passive)", "Heals your allies", false, 8);
		ULT_TELEPORT = War3_AddRaceSkill(thisRaceID, "Mass Teleport (Ultimate)", "Teleport", true, 8);
...
public OnWar3EventSpawn(client)
{
	if (War3_GetRace(client) == thisRaceID) {
		...
		W3SkillCooldownOnSpawn(thisRaceID, SKILL_SUMMON, 5.0, _);
		W3SkillCooldownOnSpawn(thisRaceID, SKILL_BLIZZARD, 5.0, _);
		War3_SetBuff(client, fMaxSpeed, thisRaceID, 1.2);
		BlizzardTimer[client] = 0.0;
		
		decl Float:spawn_pos_1[3];
		GetClientAbsOrigin(client, spawn_pos_1);
		decl Float:spawn_pos_2[3];
		GetClientAbsOrigin(client, spawn_pos_2);
		spawn_pos_2[2] = spawn_pos_2[2] + 999.0;
		
		TE_SetupBeamPoints(spawn_pos_1, spawn_pos_2, HaloSprite, BeamSprite, 0, 1, 3.0, 10.0, 5.0, 0, 1.0, { 0, 255, 255, 255 }, 50);
		TE_SendToAll();
		TE_SetupBeamRingPoint(spawn_pos_1, 0.0, 75.0, BeamSprite, BeamSprite, 0, 5, 3.0, 50.0, 1.0, { 0, 255, 255, 255 }, 50, 0);
		TE_SendToAll();
	}
	else {
	...
	if (!Silenced(client)) {
		...
		if (War3_GetRace(client) == thisRaceID && ability == 0 && pressed && IsPlayerAlive(client)) {
			if (War3_SkillNotInCooldown(client, thisRaceID, SKILL_BLIZZARD, true)) {
				...
				{
					blizcol =  { 0, 255, 255, 255 };
				}
				if (skill_blizzard > 0) {
					War3_CooldownMGR(client, 15.0, thisRaceID, SKILL_BLIZZARD, _, _);
					EmitSoundToAllAny(blizzardloop_sound, client);
					...
					TE_SetupBeamPoints(ranPos1, IceLocation[client], HaloSprite, BeamSprite, 0, 1, 1.0, 10.0, 5.0, 0, 1.0, blizcol, 50);
					TE_SendToAll();
					TE_SetupBeamRingPoint(ranPos1, 0.0, 75.0, BeamSprite, BeamSprite, 0, 5, 1.0, 50.0, 1.0, blizcol, 50, 0);
					TE_SendToAll();
					...
					TE_SetupBeamPoints(ranPos2, IceLocation[client], HaloSprite, BeamSprite, 0, 1, 1.0, 10.0, 5.0, 0, 1.0, blizcol, 50);
					TE_SendToAll();
					TE_SetupBeamRingPoint(ranPos2, 0.0, 75.0, BeamSprite, BeamSprite, 0, 5, 1.0, 50.0, 1.0, blizcol, 50, 0);
					TE_SendToAll();
					...
					TE_SetupBeamPoints(ranPos3, IceLocation[client], HaloSprite, BeamSprite, 0, 1, 1.0, 10.0, 5.0, 0, 1.0, blizcol, 50);
					TE_SendToAll();
					TE_SetupBeamRingPoint(ranPos3, 0.0, 75.0, BeamSprite, BeamSprite, 0, 5, 1.0, 50.0, 1.0, blizcol, 50, 0);
					TE_SendToAll();
					...
					TE_SetupBeamPoints(ranPos4, IceLocation[client], HaloSprite, BeamSprite, 0, 1, 1.0, 10.0, 5.0, 0, 1.0, blizcol, 50);
					TE_SendToAll();
					TE_SetupBeamRingPoint(ranPos4, 0.0, 75.0, BeamSprite, BeamSprite, 0, 5, 1.0, 50.0, 1.0, blizcol, 50, 0);
					TE_SendToAll();
				}
				else
				{
				...
				if (skill_summon > 0) {
					...
					if (foundtargets > 0) {
						...
						if (target > 0) {
							...
							fPos[2] += 30;
							TE_SetupBeamRingPoint(fPos, 0.0, 75.0, BeamSprite, BeamSprite, 0, 5, 2.0, 50.0, 1.0, { 255, 102, 0, 255 }, 50, 0);
							TE_SendToAll();
							...
							EmitSoundToAllAny(summon_sound, client);
...
{
	StopSoundAny(client, SNDCHAN_AUTO, summon_sound);
}
							...
							if (ValidPlayer(i, true) && GetClientTeam(i) != ownerteam) {
								...
								if (GetVectorDistance(BlizzardLocation[client], victimPos) < 300.0) {
									if (!W3HasImmunity(i, Immunity_Skills)) {
										...
										TE_SetupBeamPoints(IceLocation[client], victimPos, BeamSprite, BeamSprite, 0, 1, 1.0, 10.0, 5.0, 0, 1.0, blizcol, 50);
										TE_SendToAll();
										TE_SetupBeamRingPoint(victimPos, 0.0, 75.0, BeamSprite, BeamSprite, 0, 5, 1.0, 50.0, 1.0, blizcol, 50, 0);
										TE_SendToAll();
										
										EmitSoundToAllAny(blizzard_sound, i);
									}
								}
							}
						}
...
bool:TeleportPlayerView(client, Float:distance)
{
	if (client > 0) {
		if (IsPlayerAlive(client)) {
			...
			GetClientAbsOrigin(client, fPos);
			fPos[2] += 30;
			TE_SetupBeamRingPoint(fPos, 0.0, 75.0, BeamSprite, BeamSprite, 0, 5, 2.0, 50.0, 1.0, { 102, 153, 153, 255 }, 50, 0);
			TE_SendToAll();
			TeleportEntity(client, emptypos, NULL_VECTOR, NULL_VECTOR);
			EmitSoundToAllAny(teleport_sound, client);
