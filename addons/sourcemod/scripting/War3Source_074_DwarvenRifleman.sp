...
new String:takeaimSound1[]="*mora-wcs/war3source/particle_suck1.mp3";
new String:takeaimSound2[]="*mora-wcs/weapons/morabotti/explode5.mp3";
new String:takeaimSound1_FullPath[]="sound/mora-wcs/war3source/particle_suck1.mp3";
new String:takeaimSound2_FullPath[]="sound/mora-wcs/weapons/morabotti/explode5.mp3";
...
public OnMapStart()
{
	AddFileToDownloadsTable(takeaimSound1_FullPath);
	AddFileToDownloadsTable(takeaimSound2_FullPath);
	PrecacheSoundAny(takeaimSound2,false);
	PrecacheSoundAny(takeaimSound1,false);
    ...
    if(IS_PLAYER(victim)&&IS_PLAYER(attacker)&&victim>0&&attacker>0&&attacker!=victim)
    {
        ...
        if(vteam!=ateam)
        {
            ...
            if(race_attacker==thisRaceID && skill_level_gunpowder>0 && !Silenced(attacker))
            {
                ...
                {
                    EmitSoundToAllAny(takeaimSound2,attacker);
    ...
    if(race==thisRaceID && pressed && ValidPlayer(client))
    {
        ...
        if(ultLevel>0)
        {
            if(!Silenced(client))
            {
                ...
                {    
                    EmitSoundToAllAny(takeaimSound1,client);
