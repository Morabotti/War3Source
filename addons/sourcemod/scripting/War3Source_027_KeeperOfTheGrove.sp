...
new String:wardDamageSound[] = "*mora-wcs/ambient/woodSnd.mp3";
new String:wardDamageSound_FullPath[] = "sound/mora-wcs/ambient/woodSnd.mp3";
...
new Float:EntangleDistance[9] =  { 0.0, 200.0, 275.0, 350.0, 425.0, 500.0, 575.0, 650.0, 725.0 };
new Float:FoNDistance[9] =  { 0.0, 150.0, 160.0, 170.0, 180.0, 190.0, 200.0, 210.0, 220.0 };
new Ward_Damage[9] =  { 0, 1, 2, 2, 3, 3, 4, 4, 5 };
...
public OnPluginStart()
{
	...
	EntangleTimeCvar = CreateConVar("war3_nightelf_entangle_time", "4", "How long a target is entangled.");
...
public OnMapStart() {
	...
	PrecacheModel("models/props_foliage/urban_tree_giant01_medium.mdl");
...
public OnWar3LoadRaceOrItemOrdered(num)
{
	if (num == 270)
	{
		...
		SKILL_THORNS = War3_AddRaceSkill(thisRaceID, "Thorns Aura", "Deals damage to those who attack you", false, 8);
		SKILL_FON = War3_AddRaceSkill(thisRaceID, "Force of Nature (+ability)", "Creats an Ent.(+ability)", false, 8);
		SKILL_TRANQUILITY = War3_AddRaceSkill(thisRaceID, "Tranquility (+ability1)", "Causes a rain of healing energy to fall down, healing teammates.(+ability1)", false, 8);
		ULT_ENTANGLE = War3_AddRaceSkill(thisRaceID, "Entangling Roots", "Bind enemies to the ground rendering them immobile", true, 8);
		...
		if (skill_level > 0)
		{
			...
			if (War3_SkillNotInCooldown(client, thisRaceID, ULT_ENTANGLE, true)) {
				...
				for (new x = 0; x < 64; x++)
				{
					...
					effect_vec[2] += 5.0;
					TE_SetupBeamPoints(pPos, effect_vec, BeamSprite, HaloSprite, 0, 15, 1.5, 5.0, 5.0, 0, 50.0, { 0, 255, 0, 255 }, 0);
					TE_SendToAll();
					effect_vec[2] += 10.0;
					...
					EmitSoundToAllAny(entangleSound, targetList[x]);
					EmitSoundToAllAny(entangleSound, targetList[x]);
...
public OnWar3EventSpawn(client)
{
	...
	if (race == thisRaceID)
	{
		...
		Pos[2] += 10;
		TE_SetupBeamRingPoint(Pos, 0.0, 75.0, BeamSprite, HaloSprite, 0, 15, 4.0, 20.0, 3.0, { 0, 153, 0, 155 }, 10, 0);
		TE_SendToAll();
		TE_SetupBeamRingPoint(Pos, 75.0, 150.0, BeamSprite, HaloSprite, 0, 15, 4.0, 20.0, 3.0, { 102, 255, 102, 155 }, 10, 0);
		TE_SendToAll();
		TE_SetupBeamRingPoint(Pos, 150.0, 225.0, BeamSprite, HaloSprite, 0, 15, 4.0, 20.0, 3.0, { 0, 153, 0, 155 }, 10, 0);
		TE_SendToAll();
		TE_SetupBeamRingPoint(Pos, 225.0, 300.0, BeamSprite, HaloSprite, 0, 15, 4.0, 20.0, 3.0, { 102, 255, 102, 155 }, 10, 0);
		TE_SendToAll();
		TE_SetupBeamRingPoint(Pos, 300.0, 375.0, BeamSprite, HaloSprite, 0, 15, 4.0, 20.0, 3.0, { 0, 153, 0, 155 }, 10, 0);
		TE_SendToAll();
		TE_SetupBeamRingPoint(Pos, 375.0, 450.0, BeamSprite, HaloSprite, 0, 15, 4.0, 20.0, 3.0, { 102, 255, 102, 155 }, 10, 0);
		TE_SendToAll();
		TE_SetupBeamRingPoint(Pos, 450.0, 525.0, BeamSprite, HaloSprite, 0, 15, 4.0, 20.0, 3.0, { 0, 153, 0, 155 }, 10, 0);
		TE_SendToAll();
		TE_SetupBeamRingPoint(Pos, 525.0, 600.0, BeamSprite, HaloSprite, 0, 15, 4.0, 20.0, 3.0, { 100, 100, 100, 155 }, 10, 0);
		TE_SendToAll();
	}
}
		...
		if (War3_GetRace(client) == thisRaceID && ability == 0 && pressed && IsPlayerAlive(client))
		{
			...
			if (skill_level > 0)
			{
				if (CurrentWardCount[client] < WardStartingArr[1])
				{
					...
					if (conf_found)
					{
						PrintHintText(client, "You can't Summon a Tree Here...");
					}
					else
					{
						CreateWard(client);
						CurrentWardCount[client]++;
						PrintHintText(client, "You Summoned a Tree...");
						...
						if (entindex != -1)
						{
							if (GetClientTeam(client) == 2) {
								DispatchKeyValue(entindex, "model", "models/props_foliage/urban_tree_giant01_medium.mdl");
								dColor =  { 255, 51, 0, 255 };
							}
						...
						playerpos[2] -= 40; //enter your height choice there ^.^
						TeleportEntity(entindex, playerpos, NULL_VECTOR, NULL_VECTOR);
						playerpos[2] += 35;
						playerpos[2] += 20.0;
						TE_SetupBeamRingPoint(playerpos, 45.0, 44.0, BeamSprite, HaloSprite, 0, 15, 12.0, 9.0, 50.0, dColor, 0, 0);
						TE_SendToAll();
						playerpos[2] += 20.0;
						TE_SetupBeamRingPoint(playerpos, 45.0, 44.0, BeamSprite, HaloSprite, 0, 15, 12.0, 9.0, 50.0, dColor, 0, 0);
						TE_SendToAll(0.5);
				...
				{
					PrintHintText(client, "You've already summoned your Tree...");
				}
			}
		}
	}
			...
			if (GetVectorDistance(BeamXY, VictimPos) < FoNDistance[wardradiusskill]) ////ward RADIUS
			{
				...
				if (tempZ > BeamZ + WARDBELOW && tempZ < BeamZ + WARDABOVE)
				{
					...
					if (LastThunderClap[i] < GetGameTime() - 2) {
						...
						EmitSoundToAllAny(wardDamageSound, i, SNDCHAN_WEAPON);
