...
public OnWar3LoadRaceOrItemOrdered(num)
{
    if(num==780)
    {
    thisRaceID=War3_CreateNewRace("Seven Sins","sevensins");
    
    SKILL_EVASION=War3_AddRaceSkill(thisRaceID,"Pride","Evasion",false,10);
    SKILL_SPEED=War3_AddRaceSkill(thisRaceID,"Sloth","Speed",false,10);
    SKILL_HP=War3_AddRaceSkill(thisRaceID,"Gluttony","Health",false,10);
    SKILL_DMG=War3_AddRaceSkill(thisRaceID,"Wrath","Damage",false,10);
    SKILL_IMMUNITY=War3_AddRaceSkill(thisRaceID,"Envy","Spell Immunity",false,1);
    SKILL_AMMO=War3_AddRaceSkill(thisRaceID,"Lust","Bonus Ammo",false,10);
...
public OnWar3EventSpawn( client )
{
    ...
    if( race == thisRaceID && ValidPlayer( client, true ))
    {
        ...
        PrintToChat(client, "\x01Say \x04Menu_ChangeSkill \x01in console to change which of the seven sins skills you use.");
        ...
        switch (g_iSkillList[client])
        {
            case 0:
            {
                PrintToChat(client, "\x01Pride's Power: You have \x04EVASION");    
            }
            case 1:
            {
                PrintToChat(client, "\x01Sloth's Power: You have \x04SPEED");    
            }
            case 2:
            {
                PrintToChat(client, "\x01Gluttony's Power: You have \x04HEALTH");    
            }
            case 3:
            {
                PrintToChat(client, "\x01Wrath's Power: You have \x04DAMAGE");    
            }
            case 4:
            {
                PrintToChat(client, "\x01Envy's Power: You have \x04IMUNNITY");    
            }
            case 5:
            {
                PrintToChat(client, "\x01Lust's Power: You have \x04AMMO");    
            }
        }    
    }
}
...
Handle:BuildSkillMenu()
{
    ...
    AddMenuItem(menu, "mev", "Pride");
    AddMenuItem(menu, "raven", "Sloth");
    AddMenuItem(menu, "cone", "Gluttony");
    AddMenuItem(menu, "sake", "Wrath");
    AddMenuItem(menu, "gambit", "Envy");
    AddMenuItem(menu, "remy", "Lust");

    SetMenuTitle(menu, "Select what of the seven sins skill you want:");
