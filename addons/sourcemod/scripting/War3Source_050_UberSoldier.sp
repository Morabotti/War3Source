...
//Injury
new Float:InjuryChance[9] = {0.0, 0.10, 0.13, 0.16, 0.19, 0.22, 0.25, 0.28, 0.30};

//Bomber
new Bomb[MAXPLAYERS][5];
new AvailableBombs[5] =  { 0, 1, 2, 3, 4};
...
new Float:BombRadius[5] = {0.0, 80.0, 110.0, 130.0, 160.0};
new BombDamage[5] = {0, 40, 60, 80, 100};
new Explosion1, Explosion2;

// Force Field
new Float:NadeDistance[9] = {0.0, 50.0, 100.0, 150.0, 200.0, 250.0, 300.0, 350.0, 400.0};
new NadeNumber; 
new HeNade[1024]; 
new HeNadeNumber;  
new FlashNade[1024]; 
...
new Float:ProbeCooldown[9] = {0.0, 40.0, 37.0, 34.0, 31.0, 28.0, 25.0, 22.0, 19.0};
new String:ProbeSnd[]="*mora-wcs/war3source/ubersoldier/probe.mp3"; 
new String:ArmedSnd[]="*mora-wcs/war3source/ubersoldier/armed.mp3";
new String:ForceFieldSnd[]="*mora-wcs/war3source/ubersoldier/forcefield.mp3";
new String:BomberSnd[]="*mora-wcs/war3source/ubersoldier/bomber.mp3";
new String:ExplosionSnd[]="*mora-wcs/war3source/ubersoldier/explosion.mp3";
new String:InjurySnd[]="*mora-wcs/war3source/ubersoldier/injury.mp3";

new String:ProbeSnd_FullPath[]="sound/mora-wcs/war3source/ubersoldier/probe.mp3"; 
new String:ArmedSnd_FullPath[]="sound/mora-wcs/war3source/ubersoldier/armed.mp3";
new String:ForceFieldSnd_FullPath[]="sound/mora-wcs/war3source/ubersoldier/forcefield.mp3";
new String:BomberSnd_FullPath[]="sound/mora-wcs/war3source/ubersoldier/bomber.mp3";
new String:ExplosionSnd_FullPath[]="sound/mora-wcs/war3source/ubersoldier/explosion.mp3";
new String:InjurySnd_FullPath[]="sound/mora-wcs/war3source/ubersoldier/injury.mp3";
...
public OnWar3LoadRaceOrItemOrdered(num)
{
	...
	SKILL_INJURY = War3_AddRaceSkill( thisRaceID, "Injury", "Hurts enemy into arm with the result that he cannot maintain his weapon.", false, 8 );
	SKILL_BOMB = War3_AddRaceSkill( thisRaceID, "Bomber", "Plants explosives (+ability) and detonates them remotely (+ability1).", false, 4 );
	SKILL_FORCE = War3_AddRaceSkill( thisRaceID, "Force Field", "Allows you to deflect grenades.", false, 8 );
	ULT_PROBE = War3_AddRaceSkill( thisRaceID, "Remote Probe", "Allows you to preview your surroundings. (+ultimate)", true, 8 );
...
public OnMapStart()
{
	PrecacheModel("models/player/custom_player/legacy/doomslayer/doomslayer.mdl");
	AddFileToDownloadsTable("models/player/custom_player/legacy/doomslayer/doomslayer.dx90.vtx");
	AddFileToDownloadsTable("models/player/custom_player/legacy/doomslayer/doomslayer.mdl");
	AddFileToDownloadsTable("models/player/custom_player/legacy/doomslayer/doomslayer.phy");
	AddFileToDownloadsTable("models/player/custom_player/legacy/doomslayer/doomslayer.vvd");
	//AddFileToDownloadsTable("models/player/custom_player/legacy/doomslayer/doomslayer_arms.dx90.vtx");
	//AddFileToDownloadsTable("models/player/custom_player/legacy/doomslayer/doomslayer_arms.mdl");
	//AddFileToDownloadsTable("models/player/custom_player/legacy/doomslayer/doomslayer_arms.vvd");
	AddFileToDownloadsTable("materials/models/player/custom/doomslayer/exponent.vtf");
	AddFileToDownloadsTable("materials/models/player/custom/doomslayer/slayer.vmt");
	AddFileToDownloadsTable("materials/models/player/custom/doomslayer/slayer.vtf");
	AddFileToDownloadsTable("materials/models/player/custom/doomslayer/slayer_nrm.vtf");
	
	AddFileToDownloadsTable("materials/mora-wcs/effects/fire_cloud1_ubersoldier.vmt");
	AddFileToDownloadsTable("materials/mora-wcs/effects/fire_cloud2_ubersoldier.vmt");
	AddFileToDownloadsTable("materials/mora-wcs/effects/fire_cloud1_ubersoldier.vtf");
	AddFileToDownloadsTable("materials/mora-wcs/effects/fire_cloud2_ubersoldier.vtf");
	...
	Explosion1=PrecacheModel("materials/mora-wcs/effects/fire_cloud1_ubersoldier.vmt");
	Explosion2=PrecacheModel("materials/mora-wcs/effects/fire_cloud2_ubersoldier.vmt");
	
	AddFileToDownloadsTable(ProbeSnd_FullPath);
	AddFileToDownloadsTable(ArmedSnd_FullPath);
	AddFileToDownloadsTable(ForceFieldSnd_FullPath);
	AddFileToDownloadsTable(BomberSnd_FullPath);
	AddFileToDownloadsTable(ExplosionSnd_FullPath);
	AddFileToDownloadsTable(InjurySnd_FullPath);
	
	PrecacheSoundAny(InjurySnd);
	PrecacheSoundAny(BomberSnd);
	PrecacheSoundAny(ExplosionSnd);
	PrecacheSoundAny(ForceFieldSnd);
	PrecacheSoundAny(ProbeSnd);
	PrecacheSoundAny(ArmedSnd);
}
...
public OnWar3EventSpawn( client )
{
	...
	if( race == thisRaceID )
	{
		...
		SetEntityModel(client, "models/player/custom_player/legacy/doomslayer/doomslayer.mdl");
		for(new j =0; j <4;j++)
		{
			Bomb[client][j] = -1;
						...
						if(skill_lvl > 0)
						{
							if(!W3HasImmunity(victim,Immunity_Skills))
							{
								if(GetRandomFloat(0.0,1.0) <= InjuryChance[skill_lvl])
								{
									...
									EmitSoundToAllAny(InjurySnd,attacker);
									EmitSoundToAllAny(InjurySnd,victim); 			
	...
	if(ent != -1)
	{
		if(NadeType[num] == true)
		{
			...
			if(bProbeArmed[client] == true)
			{
				...
				EmitSoundToAllAny(ProbeSnd,client);
					...
					for(new n = 1; n <= HeNadeNumber; n++)
					{
						if(IsValidEntity(HeNade[n]) && HeNade[n] != -1 && HeNade[n] != Probe[i])
						{
							...
							if(GetVectorLength(GrenadeVec,false) <= NadeDistance[skill_lvl])
							{
								...
								EmitSoundToAllAny(ForceFieldSnd,i);
							}
						}
					}
		...
		if( ult_level > 0 )
		{
			if( War3_SkillNotInCooldown( client, thisRaceID, ULT_PROBE, true ) )
			{
				if(bProbeArmed[client] == false)
				{
					...
					EmitSoundToAllAny(ArmedSnd,client);
...
{		
	if(War3_GetRace(client)==thisRaceID && ability==0 && pressed && IsPlayerAlive(client))
	{
		...
		if(skill>0)
		{
			if(!Silenced(client))
			{
				if(BombNumber[client] < AvailableBombs[skill])
				{
					BombNumber[client]++;
					...
					if (Bomb[client][BombNumber[client]] > 0 && IsValidEntity(Bomb[client][BombNumber[client]]))
					{
						...
						SetEntProp(newbomb, Prop_Data, "m_nSolidType", 6);
						SetEntProp(newbomb, Prop_Data, "m_CollisionGroup", 2);
						SetEntProp(newbomb, Prop_Data, "m_usSolidFlags", 5);				
						SetEntityMoveType(newbomb, MOVETYPE_NONE);
						SetEntProp(newbomb, Prop_Data, "m_takedamage", 0);
						...
						EmitSoundToAllAny(BomberSnd,client);
...
public Detonate(any:client, any:skill_lvl, any:id)
{
	switch(id)
	{
		case 1:
		{
			...
			TE_SendToAll();		
			EmitSoundToAllAny(ExplosionSnd,Bomb[client][1]);
		...
		case 2:
		{
			...
			TE_SendToAll();		
			EmitSoundToAllAny(ExplosionSnd,Bomb[client][2]);
		...
		case 3:
		{
			...
			TE_SendToAll();		
			EmitSoundToAllAny(ExplosionSnd,Bomb[client][3]);
		...
		case 4:
		{
			...
			TE_SendToAll();
			EmitSoundToAllAny(ExplosionSnd,Bomb[client][4]);
