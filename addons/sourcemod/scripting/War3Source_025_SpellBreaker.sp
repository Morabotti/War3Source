...
new String:steal_sound[]="*mora-wcs/war3source/mask.mp3";
new String:fdback_sound[]="*mora-wcs/war3source/ability_refresh.mp3";
new String:steal_sound_FullPath[]="sound/mora-wcs/war3source/mask.mp3";
new String:fdback_sound_FullPath[]="sound/mora-wcs/war3source/ability_refresh.mp3";

//Spell Steal
new Float:StealCD[9] =  { 0.0, 32.0, 30.0, 28.0, 26.0, 24.0, 22.0, 20.0, 18.0 };

//Spell Immunity

//Feedback
new Float:FeedbackCD[9] =  { 0.0, 10.0, 9.0, 8.0, 7.0, 6.0, 5.0, 4.0, 3.0 };

//Control Magic
new Float:ControlRange[9] =  { 0.0, 150.0, 200.0, 250.0, 300.0, 350.0, 400.0, 450.0, 500.0 };
...
public OnMapStart()
{
	AddFileToDownloadsTable(steal_sound_FullPath);
	AddFileToDownloadsTable(fdback_sound_FullPath);
	
	PrecacheSoundAny(steal_sound);
	PrecacheSoundAny(fdback_sound);
...
public OnWar3LoadRaceOrItemOrdered(num)
{
	if(num==250){
		...
		SKILL_STEAL=War3_AddRaceSkill(thisRaceID,"Spell Steal (Kill)", "Take buffs from the enemy and use them yourself",false,8);
		SKILL_IMMUNITY=War3_AddRaceSkill(thisRaceID,"Spell Immunity (passive)","Spell immunity",false,1);
		SKILL_FEEDBACK=War3_AddRaceSkill(thisRaceID,"Feedback (Auto-Cast)","Add extra damage",false,8);
		ULT_CONTROL=War3_AddRaceSkill(thisRaceID,"Control Magic (Ultimate)","Control your enemy with magic REQUIRES CHANNELING",true,8);
...
public OnRaceChanged(client,oldrace,newrace)
{
	...
	if(newrace == thisRaceID){
		...
		if(skill_immunity==1){
			...
			W3SetPlayerColor(client,thisRaceID, 204, 51, 153,255); 
		}
		else
		{
			W3ResetPlayerColor(client, thisRaceID);
		}
		War3_WeaponRestrictTo(client,thisRaceID,"weapon_knife , weapon_mp7");
...
{	
	...
	if(War3_GetRace(client)==thisRaceID){
		...
		spawnpos[2]+=15;
		TE_SetupBeamRingPoint(spawnpos, 1.0, 250.0, BeamSprite, BeamSprite, 0, 5, 4.0, 50.0, 1.0, {120,0,120,255}, 50, 0);
		TE_SendToAll();
		spawnpos[2]+=15;
		TE_SetupBeamRingPoint(spawnpos, 250.0, 1.0, BeamSprite, BeamSprite, 0, 5, 4.0, 50.0, 1.0, {120,0,120,255}, 50, 0);
		TE_SendToAll();
		spawnpos[2]+=15;
		TE_SetupBeamRingPoint(spawnpos, 1.0, 125.0, BeamSprite, BeamSprite, 0, 5, 4.0, 50.0, 1.0, {120,0,120,255}, 50, 0);
		TE_SendToAll();
		spawnpos[2]+=15;
		TE_SetupBeamRingPoint(spawnpos, 125.0, 1.0, BeamSprite, BeamSprite, 0, 5, 4.0, 50.0, 1.0, {120,0,120,255}, 50, 0);
		TE_SendToAll();
	}
			...
			if(War3_SkillNotInCooldown(attacker,thisRaceID,SKILL_STEAL,true)){
				if(War3_GetRace(victim)!=War3_GetRaceIDByShortname("wisp")){
					if(skill_steal>0){
						...
						EmitSoundToAllAny(steal_sound, attacker);
						EmitSoundToAllAny(steal_sound, victim);
					}
				}
		...
		if(vteam!=ateam){
			...
			if(race_attacker==thisRaceID){
				...
				if(War3_SkillNotInCooldown(attacker,thisRaceID,SKILL_FEEDBACK,true)&&!W3HasImmunity(victim,Immunity_Skills)){
					...
					EmitSoundToAllAny(fdback_sound, victim);
				}
			}
		}
	}
