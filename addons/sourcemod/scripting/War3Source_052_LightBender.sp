...
new Float:RGBChance[9] = { 0.00, 0.10, 0.12, 0.14, 0.16, 0.18, 0.20, 0.22, 0.25 };
...
public OnWar3LoadRaceOrItemOrdered(num)
{
	...
	SKILL_RED = War3_AddRaceSkill( thisRaceID, "Red Laser: Burn", "Burn your targets", false, 8 );	
	SKILL_GREEN = War3_AddRaceSkill( thisRaceID, "Green Laser: Shake", "Shake your targets", false, 8 );	
	SKILL_BLUE = War3_AddRaceSkill( thisRaceID, "Blue Laser: Freeze", "Freeze your Targets", false, 8 );
...
public OnWar3EventSpawn( client )
{
	...
	if( currRace == thisRaceID )
	{
		...
		GetClientAbsOrigin(client, spawn_pos2);
		spawn_pos2[2] += 250;
		spawn_pos[0] += 60;
		spawn_pos[1] += 60;
		TE_SetupBeamPoints( spawn_pos2, spawn_pos, BeamSprite, HaloSprite, 0, 1, 8.0, 20.0, 2.0, 0, 0.0, { 255, 11, 15, 255 }, 1 );
		TE_SendToAll();
		GetClientAbsOrigin(client, spawn_pos);
		spawn_pos[0] += -60;
		spawn_pos[1] += 60;
		TE_SetupBeamPoints( spawn_pos2, spawn_pos, BeamSprite, HaloSprite, 0, 1, 8.0, 20.0, 2.0, 0, 0.0, { 11, 255, 15, 255 }, 1 );
		TE_SendToAll();
		GetClientAbsOrigin(client, spawn_pos);
		spawn_pos[0] += -60;
		spawn_pos[1] += -60;
		TE_SetupBeamPoints( spawn_pos2, spawn_pos, BeamSprite, HaloSprite, 0, 1, 8.0, 20.0, 2.0, 0, 0.0, { 15, 11, 255, 255 }, 1 );
		TE_SendToAll();
		GetClientAbsOrigin(client, spawn_pos);
		spawn_pos[0] += 60;
		spawn_pos[1] += -60;
		TE_SetupBeamPoints( spawn_pos2, spawn_pos, BeamSprite, HaloSprite, 0, 1, 8.0, 20.0, 2.0, 0, 0.0, { 255, 255, 0, 255 }, 1 );
		TE_SendToAll();
	}
}
	...
	if( W3GetDamageIsBullet() && ValidPlayer( victim, true ) && ValidPlayer( attacker, true ) && GetClientTeam( victim ) != GetClientTeam( attacker ) )
	{
		if( War3_GetRace( attacker ) == thisRaceID && !W3HasImmunity( victim, Immunity_Skills ))
		{
			...
			if( !Hexed( attacker, false ) && skill_red > 0 && GetRandomFloat( 0.0, 1.0 ) <= RGBChance[skill_red] )
			{
				...
				CPrintToChat( victim, "Red Laser :  Burn" );
				CPrintToChat( attacker, "Red Laser :  Burn" );
				...
				TE_SetupBeamPoints( StartPos, EndPos, BeamSprite, HaloSprite, 0, 1, 8.0, 20.0, 2.0, 0, 0.0, { 255, 11, 15, 255 }, 1 );
				TE_SendToAll();
			...
			if( !Hexed( attacker, false ) && skill_green > 0 && GetRandomFloat( 0.0, 1.0 ) <= RGBChance[skill_green] )
			{
				...
				CPrintToChat( victim, "Green Laser :  Shake" );
				CPrintToChat( attacker, "Green Laser :  Shake" );
				...
				TE_SetupBeamPoints( StartPos, EndPos, BeamSprite, HaloSprite, 0, 1, 8.0, 20.0, 2.0, 0, 0.0, { 11, 255, 15, 255 }, 1 );
				TE_SendToAll();
			...
			if( !Hexed( attacker, false ) && skill_blue > 0 && GetRandomFloat( 0.0, 1.0 ) <= RGBChance[skill_blue] )
			{
				...
				CPrintToChat( victim, "Blue Laser :  Freeze" );
				CPrintToChat( attacker, "Blue Laser :  Freeze" );
				...
				TE_SetupBeamPoints( StartPos, EndPos, BeamSprite, HaloSprite, 0, 1, 8.0, 20.0, 2.0, 0, 0.0, { 15, 11, 255, 255 }, 1 );
				TE_SendToAll();
...
stock Disco(client)
{
	...
	else
	{
		...
		PrintToChat( client, "%s will teleport to you and become a Disco Ball in 3 seconds", NameVictim );
		PrintToChat( ClientTarget[client], "You will teleport to %s and become a Disco Ball in 3 seconds", NameAttacker );
		
		War3_CooldownMGR( client, 40.0, thisRaceID, ULT_DISCO, _, _ );
	}
}
