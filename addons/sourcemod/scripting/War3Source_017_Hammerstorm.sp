...
// Storm Bolt
new BoltDamage[9] = {0,3,6,9,12,15,18,21,24};
new Float:BoltRange[9]={0.0, 100.0, 130.0, 160.0, 190.0, 220.0, 250.0, 280.0, 310.0};
new Float:BoltStunDuration=1.0;
...
new Float:CleaveMultiplier[9] = {0.0,0.1,0.15,0.20,0.25, 0.30, 0.35, 0.40, 0.40};

// Warcry Buffs
new Float:WarcrySpeed[9]={1.0, 1.06, 1.08, 1.10, 1.12, 1.14, 1.16, 1.20, 1.25};
new WarcryArmor[9]={0,1,1,2,2,3,3,4,5};

// Gods Strength
new Float:GodsStrength[9]={1.0, 1.20, 1.25, 1.30, 1.35, 1.40, 1.45, 1.50, 1.55};
new bool:bStrengthActivated[MAXPLAYERSCUSTOM];
new Handle:ultCooldownCvar; // cooldown

// Sounds
new String:hammerboltsound[] ="*mora-wcs/war3source/hammerstorm/stun.mp3";
new String:ultsnd[] ="*mora-wcs/war3source/hammerstorm/ult.mp3";
new String:hammerboltsound_FullPath[] ="sound/mora-wcs/war3source/hammerstorm/stun.mp3";
new String:ultsnd_FullPath[] ="sound/mora-wcs/war3source/hammerstorm/ult.mp3";
...
public OnWar3LoadRaceOrItemOrdered(num)
{
    if(num==170)
    {
        ...
        SKILL_BOLT=War3_AddRaceSkillT(thisRaceID,"StormBolt",false,8,"150/190/250/310","5/10/18/25");
        SKILL_CLEAVE=War3_AddRaceSkillT(thisRaceID,"GreatCleave",false,8,"10/20/30/40","150");
        SKILL_WARCRY=War3_AddRaceSkillT(thisRaceID,"Warcry",false,8,"1/2/3/4","6/9/12/15");
        ULT_STRENGTH=War3_AddRaceSkillT(thisRaceID,"GodsStrength",true,8,"20/30/40/50");
...
public OnMapStart()
{
	AddFileToDownloadsTable(hammerboltsound_FullPath);
	...
	PrecacheSoundAny(hammerboltsound);
...
public OnWar3EventSpawn(client)
{
	...
	if(race == thisRaceID)
	{
		...
		for (new i = 0; i < 5; i++)
		{
			TE_SetupBeamRingPoint(SpawnPos, 10.0, 400.0, g_BeamSprite, g_HaloSprite, 0, 25, 2.5, 5.0, 0.0, StormCol, 10, 0);
			TE_SendToAll(time);
			SpawnPos[2]+=10.0;
			TE_SetupBeamRingPoint(SpawnPos, 10.0, 400.0, g_BeamSprite, g_HaloSprite, 0, 25, 2.5, 5.0, 0.0, StormCol, 10, 0);
			TE_SendToAll(time);
