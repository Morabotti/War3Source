...
new String:PModel[MAXPLAYERS][128];
...
//skill 3
new String:crow[]="*mora-wcs/war3source/morabotti/alert3.mp3";
new String:crow_FullPath[]="sound/mora-wcs/war3source/morabotti/alert3.mp3";
...
public OnWar3LoadRaceOrItemOrdered(num)
{
	if(num==260)
	{
		...
		SKILL_FAERIE=War3_AddRaceSkill(thisRaceID,"Faerie Fire (+ability)","Mark your target. Get extra XP when they die",false,8);
...
public OnMapStart()
{
	...
	AddFileToDownloadsTable(crow_FullPath);
			...
			if(War3_SkillNotInCooldown(client,thisRaceID,SKILL_FAERIE,true))
			{
				if(!Silenced(client))
				{
					...
					if(target>0)
					{
						...
						W3SetPlayerColor(target,thisRaceID, 153, 51, 153, 80, GLOW_SKILL); 
					}
					else
					{
		...
		if(vteam!=ateam)
		{
			...
			if(race_attacker==thisRaceID && skill_level_cyclone>0)
			{
				if(GetRandomFloat(0.0,1.0)<=0.5 && !W3HasImmunity(victim,Immunity_Skills)&&!Silenced(attacker))
				{
					...
					//ClientCommand(attacker, "play *ambient/wind_moan1.wav");
...
public Action:returnform(Handle:h, any:client)
{
	if(ValidPlayer(client,true))
	{
		...
		War3_WeaponRestrictTo(client,thisRaceID,"weapon_knife");
		GivePlayerItem(client, "weapon_knife");
		SetEntityModel(client, PModel[client]);
	}
}
...
public OnWar3EventSpawn(client)
{
	...
	if(race==thisRaceID)
	{
		GetEntPropString(client, Prop_Data, "m_ModelName", PModel[client], 128);
		SetEntityModel(client, PModel[client]);
		bFlying[client]=false;
		War3_SetBuff(client,bFlyMode,thisRaceID,false);
		War3_WeaponRestrictTo(client,thisRaceID,"weapon_knife");
		...
		TE_SetupBeamRingPoint(spawnpos, 20.0, 80.0,TornadoSprite,TornadoSprite, 0, 5, 2.6, 20.0, 0.0, {54,66,120,100}, 10,FBEAM_HALOBEAM);
		TE_SendToAll();
		spawnpos[2]+=20.0;
		TE_SetupBeamRingPoint(spawnpos, 40.0, 100.0,TornadoSprite,TornadoSprite, 0, 5, 2.4, 20.0, 0.0, {54,66,120,100}, 10,FBEAM_HALOBEAM);
		TE_SendToAll();
		spawnpos[2]+=20.0;
		TE_SetupGlowSprite(spawnpos, ShieldSprite, 1.0, 1.0, 130);
		TE_SendToAll(); 
		TE_SetupBeamRingPoint(spawnpos, 60.0, 120.0,TornadoSprite,TornadoSprite, 0, 5, 2.2, 20.0, 0.0, {54,66,120,100}, 10, FBEAM_HALOBEAM);
		TE_SendToAll();
		spawnpos[2]+=20.0;
		TE_SetupBeamRingPoint(spawnpos, 80.0, 140.0,TornadoSprite,TornadoSprite, 0, 5, 2.0, 20.0, 0.0, {54,66,120,100}, 10, FBEAM_HALOBEAM);
		TE_SendToAll();	
		spawnpos[2]+=20.0;
		TE_SetupBeamRingPoint(spawnpos, 100.0, 160.0,TornadoSprite,TornadoSprite, 0, 5, 1.8, 20.0, 0.0, {54,66,120,100}, 10, FBEAM_HALOBEAM);
		TE_SendToAll();	
		spawnpos[2]+=20.0;
		TE_SetupBeamRingPoint(spawnpos, 120.0, 180.0,TornadoSprite,TornadoSprite, 0, 5, 1.6, 20.0, 0.0, {54,66,120,100}, 10, FBEAM_HALOBEAM);
		TE_SendToAll();	
		spawnpos[2]+=20.0;
		TE_SetupBeamRingPoint(spawnpos, 140.0, 200.0,TornadoSprite,TornadoSprite, 0, 5, 1.4, 20.0, 0.0, {54,66,120,100}, 10, FBEAM_HALOBEAM);
		TE_SendToAll();	
		spawnpos[2]+=20.0;
		TE_SetupBeamRingPoint(spawnpos, 160.0, 220.0,TornadoSprite,TornadoSprite, 0, 5, 1.2, 20.0, 0.0, {54,66,120,100}, 10, FBEAM_HALOBEAM);
		TE_SendToAll();	
		spawnpos[2]+=20.0;
		TE_SetupBeamRingPoint(spawnpos, 180.0, 240.0,TornadoSprite,TornadoSprite, 0, 5, 1.0, 20.0, 0.0, {54,66,120,100}, 10, FBEAM_HALOBEAM);
		TE_SendToAll();
		EmitSoundToAllAny(Tornado,client);
		//ClientCommand(client, "play *ambient/wind_moan1.wav");
	}
	else{
