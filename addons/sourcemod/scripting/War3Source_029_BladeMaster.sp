...
//skill 1
new Float:WindWalkTime[9] =  { 0.0, 4.0, 5.0, 5.5, 6.5, 7.0, 8.0, 8.5, 9.5 };
new Float:AbilityCooldownTime = 15.0;
new bool:bWinded[66];
new String:ww_on[] = "*mora-wcs/npc/scanner/scanner_nearmiss1.mp3";
new String:ww_off[] = "*mora-wcs/npc/scanner/scanner_nearmiss2.mp3";
new String:ww_on_FullPath[] = "sound/mora-wcs/npc/scanner/scanner_nearmiss1.mp3";
new String:ww_off_FullPath[] = "sound/mora-wcs/npc/scanner/scanner_nearmiss2.mp3";
...
new Float:OverloadDamageIncrease[9] =  { 1.0, 1.015, 1.02, 1.03, 1.04, 1.05, 1.06, 1.07, 1.08 };
////
new UltimateZapsRemaining[66];
new Float:PlayerDamageIncrease[66];
new WindSprite;
new String:ultsnd_029[] = "*mora-wcs/war3source/morabotti/bladecut.mp3";
new String:ultsnd_029_FullPath[] = "sound/mora-wcs/war3source/morabotti/bladecut.mp3";
...
public OnWar3LoadRaceOrItemOrdered(num)
{
	if (num == 290)
	{
		thisRaceID = War3_CreateNewRace("Blademaster", "bmaster");
...
public OnMapStart()
{
	AddFileToDownloadsTable("materials/mora-wcs/sprites/crosshairs.vmt");
	AddFileToDownloadsTable("materials/mora-wcs/sprites/crosshairs.vtf");
	
	WindSprite = PrecacheModel("materials/mora-wcs/sprites/crosshairs.vmt");
	GlowSprite = PrecacheModel("models/player/tm_leet_variantb.mdl");
	GlowSprite2 = PrecacheModel("models/player/ctm_gsg9.mdl");
	
	AddFileToDownloadsTable(ultsnd_029_FullPath);
	AddFileToDownloadsTable(ww_on_FullPath);
	AddFileToDownloadsTable(ww_off_FullPath);
	
	PrecacheSoundAny(ultsnd_029);
	PrecacheSoundAny(ww_on);
	PrecacheSoundAny(ww_off);
}
...
public OnWar3EventSpawn(client)
{
	...
	if (race == thisRaceID)
	{
		...
		new Float:spawnpos[3];
		{
			...
			{
				spawnpos[0] += 60;
				TE_SetupGlowSprite(spawnpos, GlowSprite, 4.0, 1.0, 250);
				TE_SendToAll();
				spawnpos[0] -= 120;
				TE_SetupGlowSprite(spawnpos, GlowSprite, 4.0, 1.0, 250);
				TE_SendToAll();
			...
			{
				spawnpos[0] += 60;
				TE_SetupGlowSprite(spawnpos, GlowSprite2, 4.0, 1.0, 250);
				TE_SendToAll();
				spawnpos[0] -= 120;
				TE_SetupGlowSprite(spawnpos, GlowSprite2, 4.0, 1.0, 250);
				TE_SendToAll();
		...
		if (skill_level > 0)
		{
			if (War3_SkillNotInCooldown(client, thisRaceID, SKILL_WINDWALK, true))
			{
				if (!Silenced(client))
				{
					...
					bWinded[client] = true;
					EmitSoundToAllAny(ww_on, client);
...
public Action:RemoveInvis(Handle:t, any:client)
{
	if (ValidPlayer(client, true) && bWinded[client] == true)
	{
		...
		EmitSoundToAllAny(ww_off, client);
		War3_SetBuff(client, fMaxSpeed, thisRaceID, 1.0);
	}
}
		...
		if (vteam != ateam)
		{
			...
			if (race_attacker == thisRaceID)
			{
				...
				if (bWinded[attacker] == true && !Silenced(attacker))
				{
					...
					EmitSoundToAllAny(ww_off, attacker);
				}
				//critical
		...
		if (skill > 0)
		{
			if (War3_SkillNotInCooldown(client, thisRaceID, ULT_BLADESTORM, true))
			{
				if (!Silenced(client))
				{
					...
					EmitSoundToAllAny(ultsnd_029, client);
...
public Action:UltimateLoop(Handle:timer, any:userid)
{
	...
	if (ValidPlayer(attacker) && UltimateZapsRemaining[attacker] > 0 && IsPlayerAlive(attacker))
	{
		...
		TE_SendToAll();
		EmitSoundToAllAny(ultsnd_029, attacker);
