...
public OnMapStart()
{
	AddFileToDownloadsTable("materials/mora-wcs/models/MANHACK/Blur01.vmt");
	AddFileToDownloadsTable("materials/mora-wcs/models/MANHACK/Blur01.vtf");
	BeamSprite=PrecacheModel("materials/mora-wcs/models/MANHACK/Blur01.vmt");
	CreateTimer(0.2,restore,_,TIMER_REPEAT);
	HookEvent("round_start",RoundStartEvent);
	MoneyOffsetCS=FindSendPropInfo("CCSPlayer","m_iAccount");
	BeamSprite2=PrecacheModel("materials/mora-wcs/sprites/lgtning.vmt");
	HaloSprite=PrecacheModel("materials/mora-wcs/sprites/halo01.vmt");
}

new SKILL_RESTORE, SKILL_UNSUMMON, ULT_SACRIFICE;

new HealAmt[9]={0,70,60,50,40,30,20,15, 10};
new summ[9]={0,250,300,350,400,450,500,520, 550};
new bool:bRestore[66];
new bool:bShade[66];
new Float:cool[9]={30.0, 28.0, 26.0, 24.0, 22.0, 20.0, 18.0, 17.0, 16.0};
new healarr[9]={0, 28, 26, 24, 22, 20, 18, 16, 15};
...
public OnWar3LoadRaceOrItemOrdered(num)
{
	...
	SKILL_RESTORE=War3_AddRaceSkill(thisRaceID,"Restore(ability)","Use resources to heal",false,8);
	SKILL_UNSUMMON=War3_AddRaceSkill(thisRaceID,"UnSummon(ability1)","Unsummon primary for resources",false,8);
	ULT_SACRIFICE=War3_AddRaceSkill(thisRaceID,"Sacrifice","Turns Acolyte into a shade",false,8);
			...
			if(War3_SkillNotInCooldown(client,thisRaceID,SKILL_RESTORE,true))
			{
				if(!Silenced(client))
				{
					if(!bRestore[client])
					{
						...
						iVec[2]+=15;
						TE_SetupBeamRingPoint(iVec,1.0,800.0,BeamSprite2,HaloSprite,0,15,0.3,150.0,2.0,{255,255,0,255},0,0);
						TE_SendToAll();
						TE_SetupBeamRingPoint(iVec,800.0,1.0,BeamSprite2,HaloSprite,0,15,0.5,150.0,2.0,{0,255,0,255},0,0);
						TE_SendToAll(0.3);
